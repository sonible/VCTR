                            ...
                            (float, double, int32_t, int64_t, uint32_t, uint64_t) )
{
    ...
    const vctr::Vector max = filter << vctr::elementwiseMax (srcA, srcB);
    auto reference = vctr::Vector (srcA.size(), [&] (auto n) { return std::max (srcA[n], srcB[n]); });

    REQUIRE_THAT (max, vctr::Equals (reference));
}