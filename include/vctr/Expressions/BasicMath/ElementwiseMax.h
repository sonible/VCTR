...
class ElementwiseMax : ExpressionTemplateBase
{
public:
    using value_type = std::common_type_t<ValueType<SrcAType>, ValueType<SrcBType>>;

    VCTR_COMMON_BINARY_VEC_VEC_EXPRESSION_MEMBERS (ElementwiseMax, srcA, srcB)

    VCTR_FORCEDINLINE constexpr auto operator[] (size_t i) const
    {
        return std::max (srcA[i], srcB[i]);
    }
    ...
    {
        return Expression::AVX::max (srcA.getAVX (i), srcB.getAVX (i));
    }
    ...
    {
        return Expression::SSE::max (srcA.getSSE (i), srcB.getSSE (i));
    }
    ...
    {
        return Expression::Neon::max (srcA.getNeon (i), srcB.getNeon (i));
    }
};
...
constexpr auto elementwiseMax (SrcAType&& a, SrcBType&& b)
{
    ...
    return expressions::ElementwiseMax<extent, SrcAType, SrcBType> (std::forward<SrcAType> (a), std::forward<SrcBType> (b));
}
} // namespace vctr
