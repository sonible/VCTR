...
public:
    ...
    {
        return Expression::IPP::max (src.data(), sizeToInt (size()));
    };
    ...
    //==============================================================================
    template <size_t n>
    VCTR_FORCEDINLINE static constexpr value_type finalizeReduction (const std::array<value_type, n>& maxima)
    {
        if constexpr (n == 1)
            return maxima[0];
...
public:
    ...
    {
        return Expression::IPP::maxAbs (src.data(), sizeToInt (size()));
    };
    ...
    requires Config::archX64 && has::getAVX<SrcType> && is::realFloatNumber<value_type>
    {
        ...
        result = Expression::AVX::max (result, Expression::AVX::bitwiseAndNot (src.getAVX (i), avxSignBit));
    }
    ...
    requires Config::archX64 && has::getSSE<SrcType> && is::realFloatNumber<value_type>
    {
        ...
        result = Expression::SSE::max (result, Expression::SSE::bitwiseAndNot (src.getSSE (i), sseSignBit));
    }
