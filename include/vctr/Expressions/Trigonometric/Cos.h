...
class Cos : ExpressionTemplateBase
{
public:
    using value_type = ValueType<SrcType>;

    VCTR_COMMON_UNARY_EXPRESSION_MEMBERS (Cos, src)
    ...
    VCTR_FORCEDINLINE constexpr value_type operator[] (size_t i) const
    {
        ...
        if constexpr (is::realFloatNumber<value_type>)
        {
            if (std::is_constant_evaluated())
                return gcem::cos (src[i]);
        }
#endif

        return std::cos (src[i]);
    }
    ...
    {
        Expression::Accelerate::cos (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
    ...
    {
        Expression::IPP::cos (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
...
/** Computes the cosine of each source element.
    ...
    @ingroup Expressions
 */
constexpr inline ExpressionChainBuilder<expressions::Cos> cos;
