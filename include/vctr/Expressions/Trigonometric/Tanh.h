...
class Tanh : ExpressionTemplateBase
{
public:
    using value_type = ValueType<SrcType>;

    VCTR_COMMON_UNARY_EXPRESSION_MEMBERS (Tanh, src)
    ...
    VCTR_FORCEDINLINE constexpr value_type operator[] (size_t i) const
    {
        ...
        if constexpr (is::realFloatNumber<value_type>)
        {
            if (std::is_constant_evaluated())
                return gcem::tanh (src[i]);
        }
#endif

        return std::tanh (src[i]);
    }
    ...
    {
        Expression::Accelerate::tanh (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
    ...
    {
        Expression::IPP::tanh (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
...
/** Computes the hyperbolic tangent of each source element.
    ...
    @ingroup Expressions
 */
constexpr inline ExpressionChainBuilder<expressions::Tanh> tanh;
