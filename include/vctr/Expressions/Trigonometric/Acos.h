...
class Acos : ExpressionTemplateBase
{
public:
    using value_type = ValueType<SrcType>;

    VCTR_COMMON_UNARY_EXPRESSION_MEMBERS (Acos, src)
    ...
    VCTR_FORCEDINLINE constexpr value_type operator[] (size_t i) const
    {
        ...
        if constexpr (is::realFloatNumber<value_type>)
        {
            if (std::is_constant_evaluated())
                return gcem::acos (src[i]);
        }
#endif

        return std::acos (src[i]);
    }
    ...
    {
        Expression::Accelerate::acos (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
    ...
    {
        Expression::IPP::acos (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
...
/** Computes the inverse cosine of each source element.
    ...
    @ingroup Expressions
 */
constexpr inline ExpressionChainBuilder<expressions::Acos> acos;
