...
class Sinh : ExpressionTemplateBase
{
public:
    using value_type = ValueType<SrcType>;

    VCTR_COMMON_UNARY_EXPRESSION_MEMBERS (Sinh, src)
    ...
    VCTR_FORCEDINLINE constexpr value_type operator[] (size_t i) const
    {
        ...
        if constexpr (is::realFloatNumber<value_type>)
        {
            if (std::is_constant_evaluated())
                return gcem::sinh (src[i]);
        }
#endif

        return std::sinh (src[i]);
    }
    ...
    {
        Expression::Accelerate::sinh (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
    ...
    {
        Expression::IPP::sinh (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
...
/** Computes the hyperbolic sine of each source element.
    ...
    @ingroup Expressions
 */
constexpr inline ExpressionChainBuilder<expressions::Sinh> sinh;
