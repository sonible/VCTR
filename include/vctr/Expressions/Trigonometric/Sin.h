...
class Sin : ExpressionTemplateBase
{
public:
    using value_type = ValueType<SrcType>;

    VCTR_COMMON_UNARY_EXPRESSION_MEMBERS (Sin, src)
    ...
    VCTR_FORCEDINLINE constexpr value_type operator[] (size_t i) const
    {
        ...
        if constexpr (is::realFloatNumber<value_type>)
        {
            if (std::is_constant_evaluated())
                return gcem::sin (src[i]);
        }
#endif

        return std::sin (src[i]);
    }
    ...
    {
        Expression::Accelerate::sin (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
    ...
    {
        Expression::IPP::sin (src.evalNextVectorOpInExpressionChain (dst), dst, sizeToInt (size()));
        return dst;
    }
...
/** Computes the sine of each source element.
    ...
    @ingroup Expressions
 */
constexpr inline ExpressionChainBuilder<expressions::Sin> sin;
